...
// MODIFIED: Renamed to be specific to Innate, removed bonus logic
function _updateInnateCalculatedFields() {
    if (!window.app.activeNPC || !window.ui.inputs.innateDC) return; // Removed bonus check
    // Don't recalculate stats here, rely on it being called by updateActiveNPCFromForm
    const { dc } = window.app.calculateSpellcastingDCBonus( // Use generic calculator, ignore bonus
        window.app.activeNPC.innateAbility,
        window.app.activeNPC.proficiencyBonus,
        window.app.activeNPC
    );
    // Only update if the current value matches the old calculated value, or if recalc forced
    const oldCalculated = window.app.calculateSpellcastingDCBonus(
        window.app.activeNPC.innateAbility, // Use current ability before potential change
        window.app.activeNPC.proficiencyBonus, // Use current prof before potential change
        window.app.activeNPC
    );
    if (parseInt(window.ui.inputs.innateDC.value, 10) === oldCalculated.dc) {
        window.ui.inputs.innateDC.value = dc;
        window.app.activeNPC.innateDC = dc;
    } else {
        window.app.activeNPC.innateDC = parseInt(window.ui.inputs.innateDC.value, 10) || dc; // Keep manual override
    }
    // No need to save/update viewport here, handled by caller (updateActiveNPCFromForm)
}

// NEW: Added function for Action casting fields, removed bonus logic
function _updateActionCalculatedFields() {
    if (!window.app.activeNPC || !window.ui.inputs.actionCastingDC) return; // Removed bonus check
    // Don't recalculate stats here
    const { dc } = window.app.calculateSpellcastingDCBonus( // Use generic calculator, ignore bonus
        window.app.activeNPC.actionCastingAbility,
        window.app.activeNPC.proficiencyBonus,
        window.app.activeNPC
    );
     const oldCalculated = window.app.calculateSpellcastingDCBonus(
        window.app.activeNPC.actionCastingAbility, // Use current ability before potential change
        window.app.activeNPC.proficiencyBonus, // Use current prof before potential change
        window.app.activeNPC
    );
     if (parseInt(window.ui.inputs.actionCastingDC.value, 10) === oldCalculated.dc) {
        window.ui.inputs.actionCastingDC.value = dc;
        window.app.activeNPC.actionCastingDC = dc;
    } else {
         window.app.activeNPC.actionCastingDC = parseInt(window.ui.inputs.actionCastingDC.value, 10) || dc; // Keep manual override
    }
    // No need to save/update viewport here
}
...
function _updateFormFromActiveNPC() {
    ...
    try {
        ...
        for (const key in window.ui.inputs) {
            ...
            if (key.startsWith('innate-') || key.startsWith('action-casting-')) continue;
        ...
        // window.ui.inputs.innateBonus REMOVED
        ...
        // window.ui.inputs.actionCastingBonus REMOVED
...
function _updateSpellcastingVisibility(isClearing = false) {
    ...
    // --- Regular Spellcasting ---
    if (window.ui.spellcastingHeader && window.ui.spellcastingFields && window.ui.spellcastingDivider && window.ui.actionSpellcastingFields) {
        ...
        // Toggle Action Spellcasting fields visibility based on radio button state AND hasSpellcasting
        ...
        // Placeholder: Add similar logic for Trait spellcasting fields if they exist later
...
if (window.ui) {
    window.ui.updateInnateCalculatedFields = _updateInnateCalculatedFields;
    window.ui.updateActionCalculatedFields = _updateActionCalculatedFields;
